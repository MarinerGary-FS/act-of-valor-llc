...
export default function TrustSection() {
  ...
  return (
    <section id="trust" className="w-full bg-[var(--color-surface-card)] py-20 sm:py-28 lg:py-32">
      <motion.div
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
            ...
            <ul className="space-y-4">
              {trustItems.map((item, idx) => (
                <li key={idx} className="flex items-center gap-4">
                  <div className="flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-[var(--color-brand-primary)] text-[var(--color-content-inverse)]">
          ...
          >
            <div className="w-full max-w-sm bg-gradient-to-br from-[var(--color-navy)] to-[var(--color-navy-light)] rounded-2xl p-8 sm:p-10 text-[var(--color-content-inverse)] shadow-[var(--shadow-xl)]">
              <div className="grid grid-cols-2 gap-8 sm:gap-10">
                {stats.map((stat, idx) => (
                  <div key={idx} className="text-center">
                    <p className="text-3xl sm:text-4xl font-bold text-[var(--color-brand-primary)] mb-2">
                      {stat.value}
                    </p>
              ...
              <div className="absolute -bottom-4 -right-4 w-20 h-20 bg-[var(--color-brand-primary)] opacity-5 rounded-full blur-2xl pointer-events-none" />
            </div>
          </motion.div>
        </div>
      </motion.div>
